...
void CalculatingDigits(int number, int& d1, int& d2, int& d3, int& d4);
void ComparingNumbers(int user_Num_d1, int user_Num_d2, int user_Num_d3, int user_Num_d4, int prog_Num_d1, int prog_Num_d2, int prog_Num_d3, int prog_Num_d4, int& count_Bulls, int& count_Cows);
...
void GameUpdate(int count_Bulls, int count_Cows);
...
int main()
{
	...
	unsigned int user_Guess = 0;				//This will be used by the user to guess the program number
	...
	while (rounds--) {
		...
		bool program_Guessed = false;
		 ...
		 do {
			...
			CalculatingDigits(user_Guess, user_Num_d1, user_Num_d2, user_Num_d3, user_Num_d4);
			CalculatingDigits(program_Num, prog_Num_d1, prog_Num_d2, prog_Num_d3, prog_Num_d4);
			ComparingNumbers(user_Num_d1, user_Num_d2, user_Num_d3, user_Num_d4, prog_Num_d1, prog_Num_d2, prog_Num_d3, prog_Num_d4, count_Bulls, count_Cows);
			GameUpdate(count_Bulls, count_Cows);
			...
			int program_Guess = program_Candidates[rand() % num_Candidates];
			cout << "Program's guess: " << program_Guess << endl;

			int progGuess_d1, progGuess_d2, progGuess_d3, progGuess_d4;
			CalculatingDigits(program_Guess, progGuess_d1, progGuess_d2, progGuess_d3, progGuess_d4);
			int secret_d1, secret_d2, secret_d3, secret_d4;
			CalculatingDigits(user_Secret_Num,secret_d1, secret_d2, secret_d3, secret_d4);

			int program_Bulls = 0, program_Cows = 0;
			ComparingNumbers(progGuess_d1, progGuess_d2, progGuess_d3, progGuess_d4, secret_d1, secret_d2, secret_d3, secret_d4, program_Bulls, program_Cows);
			...
			if (program_Bulls == 4) {
				...
				program_Guessed = true;
				break;
			}
			 
		
		 } while (!user_Guessed && !program_Guessed);
...
void UserInput(unsigned& user_Guess) {
	do {
		...
		cin >> user_Guess;
	} while (!CheckUniqueNumber(user_Guess) || user_Guess < 1000 || user_Guess > 9999);
}
...
void FilterCandidates(int candidates[], int& num_Candidates, int last_guess, int bulls, int cows) {
	...
	CalculatingDigits(d1, d2, d3, d4, last_guess);
	...
	for (int i = 0; i < num_Candidates; ++i) {
		...
		int temp_bulls = 0;
		int temp_cows = 0;
		CalculatingDigits(cd1, cd2, cd3, cd4, candidate);
		ComparingNumbers(cd1, cd2, cd3, cd4, d1, d2, d3, d4, temp_bulls, temp_cows);
